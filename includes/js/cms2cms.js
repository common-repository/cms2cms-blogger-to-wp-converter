...
if ($('.cms2cms-blogger-container').length > 0) {
    ...
    var init = function () {
        ...
        if ($('.active').hasClass('step-sign')) {
            ...
            $(document).on('click', '#verifySource_blogger', function (event) {
        ...
        $('#auth_submit').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.key) {
                        saveUser({
                            action: 'cms2cms_blogger_save_options',
                            accessKey: data.key,
    ...
    var toggleSignIN = function () {
        var cms2cmsBlock = $('#cms2cms_blogger_accordeon');
        var signInTabs = cms2cmsBlock.find('a.nav-tab');

        signInTabs.on('click', function (e) {
            if (!$(this).hasClass('cms2cms-blogger-real-link')) {
                e.preventDefault();
                var activeClass = 'nav-tab-active';
                if (!$(this).hasClass(activeClass)) {
                    signInTabs.removeClass(activeClass);
                    $(this).addClass(activeClass);
                    $(this).closest('.cms2cms_blogger_accordeon_item').find('form').attr('action', $(this).attr('href'));
                }
            }
        });

        var formContent = $('.cms2cms_blogger_accordeon_item_register').find('.center-content'),
            button = $('.login-reg');
        ...
        button.on('click', function () {
            ...
            $('.cms2cms-blogger-real-link').fadeToggle();
            button.parent('p.account-register').toggle();
            button.parent('p.account-login').toggle();
            $('.cms2cms-blogger-error span').parent().fadeOut();
    ...
    var checkDoneTab = function () {
        ...
        if ($('.cms2cms-blogger-container').hasClass('cms2cms_blogger_is_activated')) {
    ...
    var validateSource = function () {
        ...
        switch (true) {
            case $('#sourceUrl').val().length == 0:
                $('.cms2cms-blogger-error span').html('Url can`t be empty').parent().fadeIn();
                break;
            case !regex.test(sourceUrl):
                $('.cms2cms-blogger-error span').html('Url invalid, please enter url in "http://your_Blogger_website.com/" format').parent().fadeIn();
                break;
            case sourceUrl == targetUrl:
                $('.cms2cms-blogger-error span').html('Please, make sure to provide your source website URL (the URL of your current website)').parent().fadeIn();
                break;
            default :
                $('#sourceUrl').val(sourceUrl);
                ajaxValidate($('#verifySource_blogger').closest('form').serialize());
                return;
        }
        $('#cms_overlay').fadeOut();
    };
